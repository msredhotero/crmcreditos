...
public function __construct($monto, $modo){
	$this->monto = $monto;
	$cadena = '';
	if($modo == 1)
	$cadena = $this->covertirNumLetras();
	if($modo == 2)
	$cadena = $this->covertirPorcientoLetras();
	if($modo == 3)
	$cadena = $this->covertirNumLetrasSimple();

	$this->cadenaMonto = $cadena;


}

public function getCadenaMonto(){
	return $this->cadenaMonto;
}
...
function covertirPorcientoLetras()
{
	$number = $this->monto;
    $number1=$number;
   ...
   if ($centavos == "00")
   {
      $centavos = "";
	    ...
	    $centavos_final_string = $this->string_literal_conversion($centavos) ;    
...
function covertirNumLetrasSimple()
{
