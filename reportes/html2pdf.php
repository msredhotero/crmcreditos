...
public function SetHeaderArbol($header)
{
	$this->header = $header;
	$this->imgHead = true;
		...
		if($i%2==0)
		{
			...
			if($this->HREF){
				...
				if($a[($i-1)] == 'center' || $a[($i-1)] == 'CENTER' || $a[($i-1)] == 'centerb' || $a[($i-1)] == 'CENTERB'){
					$this->Cell(0, 5, stripslashes(txtentities2($e)), 0, 0, 'C', '', '');
					...
					$this->MultiCellDos(0,5,stripslashes(txtentities2($e)));
