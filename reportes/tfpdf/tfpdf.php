...
function MultiCells($w, $h, $txt, $border=0, $align='J', $fill=false)
{
	...
	$x_tamanio =strlen( mb_substr($s,$j,$i-$j,'UTF-8'));
	#echo "ultima linea tamanio =>".$x_tamanio;
	if ($this->unifontSubset) {
		$this->Cell($w -$x_tamanio ,$h,mb_substr($s,$j,$i-$j,'UTF-8'),$b,2,$align,$fill);
	}
	else {
		$this->Cell($w-$x_tamanio,$h,substr($s,$j,$i-$j),$b,2,$align,$fill);
	}
	$this->x = $this->lMargin;
}

function MultiCellDos($w, $h, $txt, $border=0, $align='J', $fill=false)
{
	// Output text with automatic or explicit line breaks
	if(!isset($this->CurrentFont))
		$this->Error('No font has been set');
