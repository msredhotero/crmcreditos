...
$query = new Query();
	...
	$sqlDocto = "SELECT * FROM 	dbcontratosglobalesexpedientes WHERE 	refcontratoglobal = $idContrato ";
...
$cadena2 = '';
	while($objD = $query->fetchObject($resD)){
		$idDiv = $objD->dbcontratoglobalexpediente;
		$ruta =$objD->ruta;
		...
		$cadena1 .=  'PDFObject.embed("../../../'.$documento.'", container, options);';
	}
