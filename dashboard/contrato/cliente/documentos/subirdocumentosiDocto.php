...
$resDocumentaciones = $documentos->traerDocumentacionPorTipoCreditoDocumentacionId($idContratoGlobal, 1, $iddocumento);
...
$resDocumentacionesCompletas = $documentos->traerDocumentacionPorTipoCreditoDocumentacionCompleta($idContratoGlobal,'1');

$Doctos = $documentos->traerDocumentacionPorTipoCreditoDocumentacionCompleta($idContratoGlobal,'1');
$DoctosAA = $documentos->traerDocumentacionPorTipoCreditoDocumentacionCompleta($idContratoGlobal,'1');
$siguiente = false;
$idSiguiente = '';
$doctoSiguiente = '';
...
$doctoSiguienteA ='';
$idDoctoSiguienteA ='';
while( $rowSiguiente = mysql_fetch_array($Doctos)){
 
  $idDoctoS =  $rowSiguiente['iddocumento'];
  $idDoctoActual =$rowSiguiente['iddocumento'] ;

 # echo  $rowSiguiente['documento'] .$rowSiguiente['iddocumento']."= >".$rowSiguiente['nombre']."=><br>\n";
  if($siguiente && $rowSiguiente['iddocumento']!= 22 && $rowSiguiente['iddocumento']!= 21){
    $idSiguiente = $idDoctoS ;
   
      $doctoSiguiente =  $rowSiguiente['documento']; 
    
       
  }
    if(($rowSiguiente['iddocumento']== 22 && $rowSiguiente['req'] == 1 ) || ($rowSiguiente['iddocumento']== 21 && $rowSiguiente['req'] == 1 )){
      $idSiguiente = $idDoctoS ;
      if($rowSiguiente['nombre'] != ''){
      $doctoSiguiente =  $rowSiguiente['documento'];
    }
...
$siguiente = false;
$actual ='';
while( $rowDumentos = mysql_fetch_array($DoctosAA)){ 
  $idDoctoS =  $rowDumentos['iddocumento'];
  $idDoctoActual =$rowDumentos['iddocumento'] ;
  $requerido =   $rowSiguiente['req'] ;
if($siguiente){
    $idSiguiente = $idDoctoS;
    if(($idDoctoS != 21 && $idDoctoS != 22) || ($idDoctoS == 21 && $requerido ) || ($idDoctoS == 22 && $requerido))
    $siguiente = false;
  }
  if($idDoctoS ==$iddocumento){
      $actual =  $rowDumentos['iddocumento'];
      $anterior = false;
      $siguiente = true;
    }
...
$idAnterior = ($idAnterior == $actual)?'':$idAnterior;

#echo "\n <br> Actual => :".$actual ." Siguien=>".$idSiguiente." Anterior =>".$idAnterior;
   ...
   <?php echo $baseHTML->getContentHeader (); ?>
    ...
    <section class="content">
      <div class="container-fluid">
             ...
             <div class="  col-s-6 col-xs-6">
         <?php if($idAnterior !=''){ ?>
          <button type="button" class='btn btn-block bg-green btn-xs mb-2 p-1 btnDocumentacion' id="<?php echo $idAnterior; ?>">
            <h6>Anteriror<br>
           </h6></button>
        </div>
      <?php }  ?>
<?php if($idSiguiente !=''){ ?>
        <div class="  col-s-6 col-xs-6">
          ...
          <button type="button" class='btn btn-block bg-green btn-xs mb-2 p-1 btnDocumentacion' id="<?php echo $idSiguiente ?>">
            <h6>Siguiente </h6></button>
        </div>
<?php }  ?>
						...
						<div class="body">
							...
							<form action="subir.php" id="frmFileUpload" class="dropzone" method="post" enctype="multipart/form-data">
								<div class="dz-message">
									...
									<h3><u>Haga click aquí para cargar o cambiar <?php echo mysql_result($resDocumentacion,0,'decripcion'); ?> </u></h3>
								</div>
								<div class="fallback">
			...
			init: function() {
				...
				this.on('success', function( file, resp ){
					...
					setTimeout(function(){
						...
						//location.reload();
							url = "subirdocumentosiDocto.php?id=<?php echo $idContratoGlobal;?>&documento=<?php echo $iddocumento; ?>";
							$(location).attr('href',url);
						}
					}, 3000);
				});
