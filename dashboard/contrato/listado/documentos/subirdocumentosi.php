...
$fechaIne = $dataContratoGlobal->getDato('vigenciaine');
$form = new FormularioSolicitud();
#$_GET['documento'] = 20;

#echo "<br><p>***";
$resDocId = $documentos->traerDocumentacionPorTipoCreditoDocumentacionCompletaClienteAdministracion($idContratoGlobal,2);
#echo "<br><p> \n ************* \n ";
...
$resDocumentaciones = $documentos->traerDocumentacionPorTipoCreditoDocumentacionCompletaClienteAdministracion($idContratoGlobal,2);
...
<div class="row"> 
						...
						<div class="body">
							...
							<form action="subir.php" id="frmFileUpload" class="dropzone" method="post" enctype="multipart/form-data">
								<div class="dz-message">
									...
									<h3><u>Haga click aquí para cargar una imagen o un documento PDF </u></h3>
								</div>
								<div class="fallback">
	...
	<div class="card-body">			<div class="row">
							...
							<div class="row ">
								...
								<?php if($iddocumento == 10) { ?> 
								<div class="form-group row col-lg-12 col-md-12 col-sm-12 col-xs-12">
									<div id='vigenciaINE' >
											<label for="vigenciaine" class="control-label" style="text-align:left"><p>Vigencia INE</label>
											

											<div id="div_fechanacimiento" class="form-line col-12">
												<input class="form-control strtoupper form-control-sm" type="date" name="vigenciaine" id="vigenciaine" value="<?php echo $fechaIne;?>" /></div>
												

											<p>
										<?php
										if ($usuarioRol == 1 || $usuarioRol == 2 || true) {
										?>
										<div>
										<button type="button" id="<?php echo $idContratoGlobal; ?>" class="btn btn-primary float-right guardarVigenciaIne" style="margin-left:0px;">Guardar vigencia</button></div>
										<?php } ?>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$('.guardarVigenciaIne').click(function() {
			idTable =  $(this).attr("id");
			modificarVigenciaINE($('#vigenciaine').val(),idTable);
		});
	...
	var myDropzone = new Dropzone("#archivos", {
		...
		function modificarVigenciaINE(fecha, id) {
			...
			if(fecha == ''){
				swal("Error!", "Es necesario indicar la fecha", "warning");
			}else{
				$.ajax({
				url: '../../../../ajax/ajax.php',
				type: 'POST',
				...
				success: function(data){
					if (data != 1) {
						swal("Ok!", 'Vigencia de INE actualizada ', "success");
						$('.guardarEstado').show();
						//location.reload();
					} else {
						swal("Error!", "Error al actualizar vigencia", "warning");
						$("#load").html('');
					}
				},
				//si ha ocurrido un error
